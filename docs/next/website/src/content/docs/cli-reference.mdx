...
Herdr normalizes metadata text before storage. It trims surrounding whitespace and caps `--title`, `--display-agent`, each `--state-label`, and token values at 80 characters. It removes all control characters from `--title`, `--display-agent`, and `--state-label` values. Empty normalized token values clear that key.

A token value can contain well-formed SGR color sequences such as `\x1b[32mtext\x1b[0m`. Herdr keeps these sequences and removes all other control characters. The sidebar can then render the token value as color and style.
